...
- a scope
- key-value map, since scope maps identifiers to values
...
```
sayHi in myobj is extractedSayHi alias
...
```
print to each in (to sort (5 to first in list))
```
...
"Jupiter" to Person is myobj alias
```
...
```
1, 2 to new in Rational is one_half alias
```
...
- can think of language creating infinitely many variables, e.g. `... to new in Number is 42 alias`, although breaks down because
...
```
to new in List is mylist alias
("John", "Jim", "Jill") to add in mylist
1 to get in mylist
2 to delete in mylist
```
...
```
to new in Set is myset alias
("John", "Jim", "Jill") to add in myset
"Jim" to delete in myset
```
