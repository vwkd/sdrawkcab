...
- if a function expects exactly two non-optional arguments, the function name can be written between the arguments without a `->`
<!-- todo: does this introduce conflict? -->
- can use to restore traditional bifix operators
...
- if a function expects exactly one non-optional argument, the function name can be written without a `->`
<!-- todo: does this introduce conflict? -->
...
<!-- todo: needs also postfix operators? -->
<!-- todo: what happens if white space is left out??? would interpret it as one whole identifier, e.g. `n+m` or `-n` or `PetersayHi` -->