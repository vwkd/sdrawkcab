...
- functions that are called prefix, infix, or postfix
- use `$` to denote where function name should go
- if one non-optional argument, can be before or after argument
...
- if two non-optional arguments, can be between arguments
<!-- todo: does this introduce conflicts? -->
...
- no precedence rules, must use grouping to achieve math-like semantics
<!-- rodo: does this introduce conflicts? -->
- note, must use whitespace between arguments and operator name, otherwise would interpret it as single identifier that doesn't exist, allowing it would need to forbid use of operator name in identifiers and semantics aware parsing, e.g. `n+m`, `-n`