...
- in separate "space", e.g. file
- "spaces" have hierarchy, e.g. folder hierarchy, like routes in a file-based router
...
- module has all identifier for sub modules in scope, just like in traditional language where declares functions in same file
- beware: no lambda, every running-later-function is module ❗️
